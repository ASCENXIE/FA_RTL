...
always @(posedge clk_wr or negedge rst_n) begin
    if (!rst_n)
        ptr_wr <= {($clog2(FIFO_DEPTH)+1){1'b0}};
    else if (wren)
        ptr_wr <= ptr_wr + 1'b1;
end
...
always @(posedge clk_rd or negedge rst_n) begin
    if (!rst_n)
        ptr_rd <= {($clog2(FIFO_DEPTH)+1){1'b0}};
    else if (rden)
        ptr_rd <= ptr_rd + 1'b1;
end
...
assign full  = ({~ptr_rd_gray_wrd2[$clog2(FIFO_DEPTH):$clog2(FIFO_DEPTH)-1], ptr_rd_gray_wrd2[$clog2(FIFO_DEPTH)-2:0]} == ptr_wr_gray[$clog2(FIFO_DEPTH):0]);
