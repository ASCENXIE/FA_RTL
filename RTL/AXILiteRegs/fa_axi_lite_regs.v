...
module fa_axi_lite_regs #(
    ...
    wire write_fire = (!s_axil_bvalid) &&
                      (awaddr_valid || aw_fire) &&
                      (wdata_valid  || w_fire);
        ...
        if (!rst_n) begin
            ...
            if (write_fire) begin
                case (write_addr[7:2])
                    ...
                    REG_Q_BASE_L: begin
                        reg_q_base_l <= (reg_q_base_l & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_Q_BASE_H: begin
                        reg_q_base_h <= (reg_q_base_h & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_K_BASE_L: begin
                        reg_k_base_l <= (reg_k_base_l & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_K_BASE_H: begin
                        reg_k_base_h <= (reg_k_base_h & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_V_BASE_L: begin
                        reg_v_base_l <= (reg_v_base_l & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_V_BASE_H: begin
                        reg_v_base_h <= (reg_v_base_h & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_O_BASE_L: begin
                        reg_o_base_l <= (reg_o_base_l & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_O_BASE_H: begin
                        reg_o_base_h <= (reg_o_base_h & ~write_mask) |
                                        (write_data   &  write_mask);
                    end

                    REG_STRIDE_BYTES: begin
                        reg_stride_bytes <= (reg_stride_bytes & ~write_mask) |
                                            (write_data       &  write_mask);
                    end

                    REG_NEG_LARGE: begin
                        reg_neg_large <= (reg_neg_large & ~write_mask) |
                                         (write_data    &  write_mask);
                    end

                    REG_SCALE: begin
                        reg_scale <= (reg_scale  & ~write_mask) |
                                     (write_data &  write_mask);
                    end
